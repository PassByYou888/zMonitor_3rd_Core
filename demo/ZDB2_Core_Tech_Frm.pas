...
uses
  ...
  M3C_Core, M3C_PascalStrings, M3C_Status, M3C_UnicodeMixedLib, M3C_MemoryStream, M3C_Cipher,
  M3C_DrawEngine, M3C_DrawEngine.SlowFMX, M3C_Geometry2D, M3C_Expression,
  M3C_ZDB2, M3C_IOThread, M3C_MemoryRaster;

type
  TZDB2_Core_Tech_Form = class(TForm)
    Memo: TMemo;
    pb: TPaintBox;
    Timer1: TTimer;
...
var
  ZDB2_Core_Tech_Form: TZDB2_Core_Tech_Form;
...
constructor TZDB2_Core_Tech_Form.Create(AOwner: TComponent);
var
  hnd: PIOHnd;
begin
...
destructor TZDB2_Core_Tech_Form.Destroy;
begin
  DisposeObject(dIntf);
  DisposeObject(ZDB);
  DisposeObject(bk);
...
procedure TZDB2_Core_Tech_Form.Button_Print_IDClick(Sender: TObject);
var
...
procedure TZDB2_Core_Tech_Form.Button_resetClick(Sender: TObject);
var
  hnd: PIOHnd;
begin
  ZDB.Free;
...
procedure TZDB2_Core_Tech_Form.Button_queryClick(Sender: TObject);
var
  ID: TZDB2_BlockHandle;
  i: Integer;
  n: TZDB2_Mem;
...
procedure TZDB2_Core_Tech_Form.Button_flushClick(Sender: TObject);
begin
  ZDB.Save;
end;

procedure TZDB2_Core_Tech_Form.DoStatus_Bcakcall(Text_: SystemString; const ID: Integer);
begin
  Memo.Lines.Add(Text_);
  Memo.GoToTextEnd;
end;

procedure TZDB2_Core_Tech_Form.EditButton_write_Click(Sender: TObject);
var
  Mem: TZDB2_Mem;
  ID: Integer;
begin
...
procedure TZDB2_Core_Tech_Form.EditButton_fill_spaceClick(Sender: TObject);
var
  Mem: TZDB2_Mem;
  ID: Integer;
begin
...
procedure TZDB2_Core_Tech_Form.EditButton_removeClick(Sender: TObject);
var
  ID: Integer;
begin
...
procedure TZDB2_Core_Tech_Form.pbMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Single);
begin
  mouse_pt := vec2(X, Y);
end;

procedure TZDB2_Core_Tech_Form.pbPaint(Sender: TObject; Canvas: TCanvas);
const
  c_Metric = 10; // 呈现器度量单位
  c_Edge = 2;    // 呈现器边缘尺度
var
...
procedure TZDB2_Core_Tech_Form.Timer1Timer(Sender: TObject);
begin
  DrawPool.Progress();
  M3C_Core.CheckThreadSynchronize;
  Invalidate;
end;
