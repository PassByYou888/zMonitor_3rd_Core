...
{ * FFMPEG Reader                                                              * }
{ ****************************************************************************** }
unit M3C_FFMPEG.Reader;

{$I M3C_Define.inc}

interface

uses SysUtils, Classes, M3C_Core, M3C_PascalStrings, M3C_UPascalStrings, M3C_UnicodeMixedLib, M3C_MemoryStream, M3C_MemoryRaster, M3C_Status, M3C_FFMPEG;
...
uses M3C_h264, M3C_FFMPEG.Writer, M3C_Geometry2D;
...
begin
  ...
  try
    ...
    if (avformat_open_input(@FFormatCtx, PAnsiChar(p), nil, @AV_Options) <> 0) then
      begin
        RaiseInfo('Could not open source %s', [VideoSource_.Text]);
        exit;
      end;
  ...
  begin
    ...
    while R >= 0 do
      begin
        ...
        // check Z.FFMPEG color conversion and scaling
