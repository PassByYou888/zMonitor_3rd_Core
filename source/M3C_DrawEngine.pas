...
{$IFDEF FPC}
  M3C_FPC.GenericList,
{$ENDIF FPC}
  M3C_Core, M3C_Cadencer, M3C_Geometry2D, M3C_Geometry.Low, M3C_Geometry3D, M3C_UnicodeMixedLib,
  M3C_HashList.Templet,
  M3C_ListEngine, M3C_TextDataEngine, M3C_MemoryRaster, M3C_PascalStrings, M3C_UPascalStrings,
  M3C_Parsing, M3C_Expression, M3C_OpCode,
  M3C_MemoryStream, M3C_Notify, M3C_BulletMovementEngine;
...
{$REGION 'Core'}
  ...
  public
    ...
    procedure PostScrollText(LifeTime: Double; Text_: SystemString; Size: Integer; COLOR, BK: TDEColor); overload;
    procedure PostScrollText(LifeTime: Double; Text_: SystemString; Size: Integer; COLOR: TDEColor); overload;
    procedure PostScrollText(Tag: TCore_Object; LifeTime: Double; Text_: SystemString; Size: Integer; COLOR, BK: TDEColor); overload;
    procedure PostScrollText(Tag: TCore_Object; LifeTime: Double; Text_: SystemString; Size: Integer; COLOR: TDEColor); overload;
...
function DEColor(const C: TRasterColor; const Alpha: TDEFloat): TDEColor; overload;
function DEColor2RasterColor(const C: TDEColor): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}

function DColor2RColor(const C: TDEColor): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function RColor2DColor(const C: TRasterColor): TDEColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
...
function DEColor(const C: TRasterColor; const Alpha: TDEFloat): TDEColor;
begin
  Result := DEColor(C);
  Result[3] := Alpha;
end;
...
begin
  Result := M3C_Geometry2D.IsEqual(Left, Dest.Left) and
    M3C_Geometry2D.IsEqual(Top, Dest.Top) and
    M3C_Geometry2D.IsEqual(Right, Dest.Right) and
    M3C_Geometry2D.IsEqual(Bottom, Dest.Bottom) and
    M3C_Geometry2D.IsEqual(Angle, Dest.Angle);
end;

function TDE4V.IsZero: Boolean;
begin
  Result :=
    M3C_Geometry2D.IsZero(Left) and
    M3C_Geometry2D.IsZero(Top) and
    M3C_Geometry2D.IsZero(Right) and
    M3C_Geometry2D.IsZero(Bottom);
end;
...
begin
  Result := M3C_Geometry2D.TV2Rect4.Init(MakeRectV2, Angle).BoundRect;
end;

function TDE4V.Centroid: TDEVec;
begin
  Result := M3C_Geometry2D.TV2Rect4.Init(MakeRectV2, Angle).Centroid;
end;
...
begin
  Result := M3C_Geometry3D.Distance(MakeRectV2, Dest.MakeRectV2);
end;

function TDE4V.GetAngleDistance(Dest: TDE4V): TDEFloat;
begin
  Result := M3C_Geometry3D.AngleDistance(Angle, Dest.Angle);
end;
...
var
  d1, d2: Double;
begin
  d1 := M3C_Geometry3D.AngleRollDistanceDeltaTime(Angle, Dest.Angle, rSpeed);
  d2 := M3C_Geometry3D.MovementDistanceDeltaTime(MakeRectV2, Dest.MakeRectV2, mSpeed);
  if d1 > d2 then
      Result := d1
  else
      Result := d2;
...
begin
  ...
  if TexturePool <> nil then
    begin
      TexturePool.FCritical__.Lock;
      Ptr_QueueStruct__ := TexturePool.Add(Self);
      TexturePool.FCritical__.UnLock;
    end;
end;
      ...
      if not FIsShadow then
        begin
          ...
          for i := (width * height) - 1 downto 0 do
              bits_^[i] := RColor(0, 0, 0, TRasterColorEntry(Bits^[i]).a);
      ...
      if PrepareDraw and (Size > 5) then
        begin
          ...
          if (FStartDrawShadowIndex >= 0) then
            begin
              Data^.r := M3C_Geometry2D.RectOffset(Data^.r, FScreenShadowOffset);
...
procedure TDrawEngine.PostScrollText(LifeTime: Double; Text_: SystemString; Size: Integer; COLOR, BK: TDEColor);
var
  n: U_String;
...
begin
  ...
  if FMaxScrollText <= 0 then
      exit;
  while FScroll_Text_Pool.Num > FMaxScrollText do
      FScroll_Text_Pool.Next;
end;

procedure TDrawEngine.PostScrollText(LifeTime: Double; Text_: SystemString; Size: Integer; COLOR: TDEColor);
begin
  PostScrollText(LifeTime, Text_, Size, COLOR, DEColor(0, 0, 0, 0));
end;

procedure TDrawEngine.PostScrollText(Tag: TCore_Object; LifeTime: Double; Text_: SystemString; Size: Integer; COLOR, BK: TDEColor);
var
  n: U_String;
  Sour: TScroll_Text_Data_Source;
begin
...
procedure TDrawEngine.PostScrollText(Tag: TCore_Object; LifeTime: Double; Text_: SystemString; Size: Integer; COLOR: TDEColor);
begin
  PostScrollText(Tag, LifeTime, Text_, Size, COLOR, DEColor(0, 0, 0, 0));
end;
            ...
            begin
              if deltaTime > 0 then
                begin
                  ...
                  st.BKColor[3] := st.BKColor[3] - st.BKColor[3] * (deltaTime / st.LifeTime);
                end;
...
Hooked_OnCheckThreadSynchronize := M3C_Core.OnCheckThreadSynchronize;
M3C_Core.OnCheckThreadSynchronize := {$IFDEF FPC}@{$ENDIF FPC}DoCheckThreadSynchronize;
Draw_Engine_Auto_Hook_Check_Thread := False;

finalization

M3C_Core.OnCheckThreadSynchronize := Hooked_OnCheckThreadSynchronize;
