...
uses SysUtils, Classes,
  M3C_ListEngine, M3C_PascalStrings, M3C_UPascalStrings, M3C_UnicodeMixedLib, M3C_TextDataEngine,
  M3C_Json,
  M3C_Core, M3C_MemoryStream, M3C_ZDB.ObjectData_LIB, M3C_ZDB,
  M3C_DFE, M3C_ZDB.ItemStream_LIB;
...
uses M3C_MH_ZDB, M3C_Cipher, M3C_Status;
  ...
  if M <> nil then
    begin
      ...
      try
        ...
        Result.MemoryUsed := M3C_MH_ZDB.GetHookMemorySize;
        if AllowedCache then
            ProcessNewInstanceCache(StorePos, Result, Result.MemoryUsed);
      finally
          M3C_MH_ZDB.EndMemoryHook;
      end;
    end;
end;
...
begin
  if AllowedCache then
    begin
      ...
      t.Buff := Buff;
      t.MemoryUsed := M3C_MH_ZDB.GetHookMemorySize;
      M3C_MH_ZDB.EndMemoryHook;
