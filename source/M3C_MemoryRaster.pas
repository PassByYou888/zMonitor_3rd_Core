...
uses Types, Math, Variants, TypInfo,
  M3C_Core, M3C_MemoryStream, M3C_Geometry2D, M3C_Geometry3D,
...
{$ELSE FPC}
  System.IOUtils,
{$ENDIF FPC}
  M3C_ListEngine, M3C_HashList.Templet, M3C_Line2D.Templet,
  M3C_Agg.Basics, M3C_Agg, M3C_Agg.Color32,
  M3C_JLS.Codec, M3C_MemoryRaster.JPEG.Type_LIB, M3C_MemoryRaster.JPEG.Image_LIB;
...
{$REGION 'MemoryRaster'}
  ...
  TMemoryRaster = class(TCore_Object)
  private
    FIsChanged: Boolean;
  ...
  public
    ...
    function IsMapFrom(raster: TMemoryRaster): Boolean;
    ...
    { save stream format }
    procedure SaveToStream(stream: TCore_Stream; RasterSave_: TRasterSaveFormat); overload; // selected format
  ...
  public
    ...
    constructor Create(AutoFreeRaster_: Boolean); overload;
    destructor Destroy; override;
    procedure Lock;
    procedure UnLock;
  ...
  TMemoryRasterList_Hash_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TString_Big_Hash_Pair_Pool<TMemoryRasterList>;

  TMemoryRasterList_Hash_Pool = class(TMemoryRasterList_Hash_Pool_Decl)
  public
    AutoFreeRaster: Boolean;
    constructor Create; overload;
    constructor Create(AutoFreeRaster_: Boolean); overload;
    procedure DoFree(var Key: SystemString; var Value: TMemoryRasterList); override;
    function Compare_Value(const Value_1, Value_2: TMemoryRasterList): Boolean; override;
...
{$REGION 'Rasterization Vertex'}
  ...
  TRasterVertex = class(TCore_Object)
  private type
  ...
  public
    ...
    *)
    ...
    (*
      ...
      Sampler: Z.MemoryRaster or Solid color
...
{$REGION 'TFontRaster'}
  ...
  TFontRaster = class(TCore_Object)
  public type
...
{$ENDIF FPC}
  ...
  public
    ...
    procedure ExportRaster(stream: TCore_Stream; partitionLine: Boolean); overload;
...
function RandomRasterColor(rand: TRandom; const A_, Min_, Max_: Byte): TRColor; overload;
function RandomRasterColor(const A_, Min_, Max_: Byte): TRColor; overload;
function RandomRasterColor(const A: Byte): TRColor; overload;
function RandomRasterColor(): TRColor; overload;
function RandomRasterColorF(const minR, maxR, minG, maxG, minB, maxB, minA, maxA: TGeoFloat): TRColor; overload;
function RandomRasterColorF(rand: TRandom; minR, maxR, minG, maxG, minB, maxB, minA, maxA: TGeoFloat): TRColor; overload;
function RasterColor(const v: TVec4): TRColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
function RasterColor(const R, G, B, A: Byte): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
function RasterColor(const R, G, B: Byte): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
...
function RasterColorF(const R, G, B, A: TGeoFloat): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
function RasterColorF(const R, G, B: TGeoFloat): TRasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
procedure RasterColor2F(const C: TRasterColor; var R, G, B, A: TGeoFloat); {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
procedure RasterColor2F(const C: TRasterColor; var R, G, B: TGeoFloat); {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}overload;
...
function BuildSequenceFrame(bmp32List: TCore_ListForObj; Column: Integer; Transparent: Boolean): TSequenceMemoryRaster;
...
procedure jls_RasterToRaw3(ARaster: TMemoryRaster; RawStream: TCore_Stream);
procedure jls_RasterToRaw1(ARaster: TMemoryRaster; RawStream: TCore_Stream);
procedure jls_GrayRasterToRaw1(const ARaster: PByteRaster; RawStream: TCore_Stream);
function EncodeJpegLSRasterToStream3(ARaster: TMemoryRaster; const stream: TCore_Stream): Boolean;
function EncodeJpegLSRasterToStream1(ARaster: TMemoryRaster; const stream: TCore_Stream): Boolean; overload;
function DecodeJpegLSRasterFromStream(const stream: TCore_Stream; ARaster: TMemoryRaster): Boolean;
function EncodeJpegLSGrayRasterToStream(const ARaster: PByteRaster; const stream: TCore_Stream): Boolean; overload;
function DecodeJpegLSGrayRasterFromStream(const stream: TCore_Stream; var ARaster: TByteRaster): Boolean;
...
procedure YV12ToRasterization(sour: TCore_Stream; dest: TMemoryRaster);
procedure RasterizationToYV12(Compressed: Boolean; sour: TMemoryRaster; dest: TCore_Stream);

{
  Half YV12
}
procedure HalfYUVToRasterization(sour: TCore_Stream; dest: TMemoryRaster);
procedure RasterizationToHalfYUV(Compressed: Boolean; sour: TMemoryRaster; dest: TCore_Stream);

{
  quart YV12
}
procedure QuartYUVToRasterization(sour: TCore_Stream; dest: TMemoryRaster);
procedure RasterizationToQuartYUV(Compressed: Boolean; sour: TMemoryRaster; dest: TCore_Stream);

{
  byte raster
}
procedure SaveByteRasterToStream(raster: TByteRaster; stream: TCore_Stream);
procedure LoadByteRasterFromStream(var raster: TByteRaster; stream: TCore_Stream);

{
  word raster
}
procedure SaveWordRasterToStream(raster: TWordRaster; stream: TCore_Stream);
procedure LoadWordRasterFromStream(var raster: TWordRaster; stream: TCore_Stream);
...
var
  ...
  NewRasterFromStream: function(const stream: TCore_Stream): TMemoryRaster;
...
uses M3C_h264.Common, M3C_Compress, M3C_Status, M3C_DFE, M3C_MemoryRaster.JPEG, M3C_Raster.PNG, M3C_Expression, M3C_OpCode, M3C_DrawEngine;
...
{$I M3C_MemoryRaster.SigmaGaussian.inc}
{$I M3C_MemoryRaster.RasterClass.inc}
{$I M3C_MemoryRaster.Agg.inc}
{$I M3C_MemoryRaster.API.inc}
{$I M3C_MemoryRaster.SequenceClass.inc}
{$I M3C_MemoryRaster.Vertex.inc}
{$I M3C_MemoryRaster.Font.inc}
{$I M3C_MemoryRaster.Morphomatics.inc}
{$I M3C_MemoryRaster.MorphologyBinaryzation.inc}
{$I M3C_MemoryRaster.MorphologySegmentation.inc}
{$I M3C_MemoryRaster.MorphologyRCLines.inc}
...
function NewRasterFromStream_(const stream: TCore_Stream): TMemoryRaster;
var
  m64: TMS64;
begin
  Result := NewRaster();
