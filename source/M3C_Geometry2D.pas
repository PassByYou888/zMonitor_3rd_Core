...
{$IFDEF FPC}
  M3C_FPC.GenericList,
{$ENDIF FPC}
  M3C_Core, Types, M3C_MemoryStream, M3C_PascalStrings, M3C_UPascalStrings, M3C_UnicodeMixedLib;
...
function MaxF(const v1, v2: TGeoFloat): TGeoFloat; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
...
uses M3C_Geometry3D, M3C_Geometry.Low, M3C_DFE, M3C_Status;
...
{$I M3C_Geometry.Split.Header.inc}
{$I M3C_Geometry.Split.Body.inc}
{$IFDEF FPC}
...
begin
  Result :=
    M3C_Geometry2D.IsZero(LeftTop) and
    M3C_Geometry2D.IsZero(RightTop) and
    M3C_Geometry2D.IsZero(RightBottom) and
    M3C_Geometry2D.IsZero(LeftBottom);
end;
...
begin
  Result := M3C_Geometry2D.BoundRect(LeftTop, RightTop, RightBottom, LeftBottom);
end;
...
begin
  if length(buff) <> 4 then
    begin
      Result := TV2Rect4.Init(M3C_Geometry2D.BoundRect(buff));
    end
  else
    with Result do
      begin
