...
{ * picture viewer tool                                                        * }
{ ****************************************************************************** }
unit M3C_DrawEngine.PictureViewer;
...
{$IFDEF FPC}
  M3C_FPC.GenericList,
{$ENDIF FPC}
  M3C_PascalStrings, M3C_UPascalStrings, M3C_UnicodeMixedLib, M3C_Status,
  M3C_DrawEngine, M3C_Geometry2D, M3C_Geometry3D, M3C_Parsing,
  M3C_MemoryRaster, M3C_MemoryStream;
        ...
        if not sData.Raster.IsEmpty then
          begin
            if FShowPictureInfo then
              begin
                tex_pt := FDrawEng.SceneToScreen(sData.DrawBox[0]);
                if sData.TexInfo <> '' then
                  begin
                    ...
                    if RectInRect(r, FDrawEng.SceneToScreen(sData.DrawBox)) then
                      begin
                        FDrawEng.Draw_BK_Text(n, FShowPictureInfoFontSize, r, DEColor(1, 1, 1), DEColor(0, 0, 0, 0.5), False);
                        tex_pt[1] := r[1, 1];
                      end;
                  end;
                ...
                if sData.TexInfo2 <> '' then
                  begin
                    ...
                    if RectInRect(r, FDrawEng.SceneToScreen(sData.DrawBox)) then
                      begin
                        FDrawEng.Draw_BK_Text(n, FShowPictureInfoFontSize2, r, DEColor(1, 1, 1), DEColor(0, 0, 0, 0.5), False);
                        tex_pt[1] := r[1, 1];
                      end;
                  end;
                ...
                if sData.TexInfo2 <> '' then
                  begin
                    ...
                    if RectInRect(r, FDrawEng.SceneToScreen(sData.DrawBox)) then
                      begin
                        FDrawEng.Draw_BK_Text(n, FShowPictureInfoFontSize3, r, DEColor(1, 1, 1), DEColor(0, 0, 0, 0.5), False);
                        tex_pt[1] := r[1, 1];
                      end;
                  end;
              end;
