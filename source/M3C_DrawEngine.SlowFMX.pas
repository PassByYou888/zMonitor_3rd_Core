...
uses System.Math.Vectors, System.Math,
  ...
  M3C_Core, M3C_PascalStrings, M3C_UPascalStrings, M3C_Geometry3D, M3C_ListEngine,
  M3C_DrawEngine, M3C_UnicodeMixedLib, M3C_Geometry2D, M3C_MemoryRaster;
...
procedure LoadMemoryBitmap(stream: TCore_Stream; raster: TMemoryRaster); overload;

procedure SaveMemoryBitmap(f: SystemString; raster: TMemoryRaster); overload;
procedure SaveMemoryBitmap(raster: TMemoryRaster; fileExt: SystemString; DestStream: TCore_Stream); overload;
procedure SaveMemoryBitmap(raster: TSequenceMemoryRaster; fileExt: SystemString; DestStream: TCore_Stream); overload;
...
{$ENDIF}
  M3C_Status, M3C_MemoryStream, M3C_MediaCenter;
...
procedure LoadMemoryBitmap(stream: TCore_Stream; raster: TMemoryRaster);
var
  Surf: TBitmapSurface;
begin
...
procedure SaveMemoryBitmap(raster: TMemoryRaster; fileExt: SystemString; DestStream: TCore_Stream);
var
  Surf: TBitmapSurface;
begin
...
procedure SaveMemoryBitmap(raster: TSequenceMemoryRaster; fileExt: SystemString; DestStream: TCore_Stream);
var
  Surf: TBitmapSurface;
begin
  if umlMultipleMatch(['.bmp'], fileExt) then
...
begin
  ...
  if FDebug then
    begin
      ...
      FCanvas.DrawRect(M3C_Geometry2D.TV2Rect4.Init(newDest.MakeRectV2, newDest.angle).BoundRectf, 0, 0, [], 0.3);
    end;
end;
...
function _NewRasterFromStream(const stream: TCore_Stream): TMemoryRaster;
begin
  Result := NewRaster();
