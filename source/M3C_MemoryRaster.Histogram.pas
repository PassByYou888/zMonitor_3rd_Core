...
unit M3C_MemoryRaster.Histogram;

{$I M3C_Define.inc}

interface

uses M3C_Core, M3C_MemoryRaster, M3C_Geometry2D, Z.Learn.Type_LIB;
  ...
  begin
    for cellX := 1 to nCX do
      begin
        ...
        NW[0] := Z.Learn.AP_Sqr(HP^.normH + ori[cellY - 1, cellX].normH + ori[cellY, cellX - 1].normH + ori[cellY - 1, cellX - 1].normH);
        NW[1] := Z.Learn.AP_Sqr(HP^.normH + ori[cellY + 1, cellX].normH + ori[cellY, cellX - 1].normH + ori[cellY + 1, cellX - 1].normH);
        NW[2] := Z.Learn.AP_Sqr(HP^.normH + ori[cellY + 1, cellX].normH + ori[cellY, cellX + 1].normH + ori[cellY + 1, cellX + 1].normH);
        NW[3] := Z.Learn.AP_Sqr(HP^.normH + ori[cellY - 1, cellX].normH + ori[cellY, cellX + 1].normH + ori[cellY - 1, cellX + 1].normH);
        ...
        if AntiLight then
          begin
            ...
            for n := 0 to length(HP^.feat) - 1 do
                LAdd(Sum, HP^.feat[n]);
            for n := 0 to length(HP^.feat) - 1 do
                HP^.feat[n] := Z.Learn.AP_Sqr(LSafeDivF(HP^.feat[n], Sum));
          end;
      end;
  end;
{$ENDIF FPC}
