...
uses Types, M3C_Core,
  M3C_Geometry.Low, M3C_Geometry2D, M3C_PascalStrings, M3C_UPascalStrings, M3C_UnicodeMixedLib;
...
begin
  Result.Buff := M3C_Geometry.Low.MatrixMultiply(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := Buff;
  det := M3C_Geometry.Low.MatrixDeterminant(Result.Buff);
  if Abs(det) < Epsilon then
      Result.Buff := M3C_Geometry.Low.IdentityHmgMatrix
  else
    begin
      M3C_Geometry.Low.AdjointMatrix(Result.Buff);
      M3C_Geometry.Low.ScaleMatrix(Result.Buff, 1 / det);
    end;
end;
...
begin
  Result.Buff := Buff;
  M3C_Geometry.Low.TranslateMatrix(Result.Buff, v);
end;

function TMatrix4.Normalize: TMatrix4;
begin
  Result.Buff := Buff;
  M3C_Geometry.Low.NormalizeMatrix(Result.Buff);
end;

function TMatrix4.Transpose: TMatrix4;
begin
  Result.Buff := Buff;
  M3C_Geometry.Low.TransposeMatrix(Result.Buff);
end;

function TMatrix4.AnglePreservingInvert: TMatrix4;
begin
  Result.Buff := M3C_Geometry.Low.AnglePreservingMatrixInvert(Buff);
end;

function TMatrix4.Determinant: TGeoFloat;
begin
  Result := M3C_Geometry.Low.MatrixDeterminant(Buff);
end;
...
begin
  Result.Buff := M3C_Geometry.Low.Pitch(Buff, angle);
end;

function TMatrix4.Roll(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := M3C_Geometry.Low.Roll(Buff, angle);
end;

function TMatrix4.Turn(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := M3C_Geometry.Low.Turn(Buff, angle);
end;
...
begin
  Result.Buff := M3C_Geometry.Low.VectorTransform(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := M3C_Geometry.Low.VectorTransform(Lhs.Buff, Rhs);
end;

class operator TVector4.Multiply(const Lhs: TMat4; const Rhs: TVector4): TVector4;
begin
  Result.Buff := M3C_Geometry.Low.VectorTransform(Rhs.Buff, Lhs);
end;
...
begin
  Result.Buff := M3C_Geometry.Low.VectorTransform(Rhs.Buff, Lhs.Buff);
end;
